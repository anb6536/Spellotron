...
//
// project.ino  
// Description: Robot maze project
// Name: Austin Theodoroff, Aahish Balimane
// Date: 12/5/18
// Class: CMPE-110
// Section: 4, Wednesday, 7:00
//
// ===========================================================
//
#include <RedBot.h>
...
// Inputs:      threshold - 1 (very light) to 1024 (very dark)
// Returns:     None
void followLine(unsigned long threshold)
{
  int     lLineValue = 0; // left line sensor value
  int     cLineValue = 0; // center line sensor value
  int     rLineValue = 0; // right line sensor value
  ...
  if ((lLineValue != prevlValue) || (cLineValue != prevcValue) || (rLineValue != prevrValue))
  {
    ...
    else if (rLineValue>threshold && cLineValue>threshold && lLineValue>threshold+50)
    {
      turnLeft( 120 );
      waitEncoderLeft( 110 );
      clearEncoders();
...
{
  pressToStart();
  while (checkUltraTooClose(620) == false)
  {
    fwd( 100 );
  }
  ...
  while (true)
  {
    if (checkUltraTooClose(620)==false)
      followLine(750);
    else
      wait(1000);      
  }
